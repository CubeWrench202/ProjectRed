...
class BusConverter(gate:BundledGatePart) extends BundledGateLogic(gate)
{
    ...
    override def save(tag:NBTTagCompound)
    {
        tag.setByte("in", rsIn.asInstanceOf[Byte])
        tag.setByte("out", rsOut.asInstanceOf[Byte])
        tag.setByte("in0", bIn.asInstanceOf[Byte])
        tag.setByte("out0", bOut.asInstanceOf[Byte])
    }
