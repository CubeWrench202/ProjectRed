...
object ComponentStore
{
    ...
    def loadBase(name:String) =
    {
        ...
        //inset deach face a little for things like posts that render overtop
    ...
    {
        val xs = new Array[TWireModel](count)
        for (i <- 0 until count) xs(i) = generateWireModel(name+"-"+i)
        xs
    }
